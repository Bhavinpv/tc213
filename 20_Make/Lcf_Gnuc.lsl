...
LCF_INTVEC0_START  = 0x80060000;
LCF_TRAPVEC0_START = 0x80020100;

LCF_PTAG_START = 0x701C0000;
LCF_PTAG_SIZE = 0xC00;

LCF_ASIL_VAR_START        = 0x70000000;
LCF_PMUECC_TVT_START      = 0x70001500;
LCF_PMUECC_RAMCODE_START  = 0x70001600;
LCF_QM_VAR_START          = 0x70002A00;
LCF_QM_PMA_VAR_START      = 0x70004000;
LCF_DEFAULT_VAR_START     = 0x70004500;

LCF_ISR15us_START             = 0x70100100;
...
MEMORY
{
  ...
  dsram0 (w!xp):        org = 0x70000000, len = 48K
  psram0 (w!xp):        org = 0x70100000, len = 8K
  psram_local (w!xp):   org = 0xc0000000, len = 8K

  /*pfls0_tstPattern(rx!p):    org = 0x80018000, len = 16K*/
  pfls0_tstPattern(rx!p):    org = 0x80058000, len = 800
  pfls0 (rx!p):              org = 0x80020000, len = 384K
  /*pfls0_tstPattern_nc(rx!p): org = 0xa0018000, len = 16K*/
  pfls0_tstPattern_nc(rx!p): org = 0xa0058000, len = 800
  pfls0_nc (rx!p):           org = 0xa0020000, len = 384K

  dfls0 (rx!p):         org = 0xaf000000, len = 1M

  edmem (w!xp):         org = 0x9f000000, len = 256K
  edmem_nc (w!xp):      org = 0xbf000000, len = 256K
}
__START_PSPR = ORIGIN(psram0);
__SIZE_PSPR = LENGTH(psram0);
__START_DSPR = ORIGIN(dsram0);
__SIZE_DSPR = LENGTH(dsram0);
...
REGION_MAP( CPU0 , ORIGIN(psram_local), LENGTH(psram_local), ORIGIN(psram0))
REGION_ALIAS( prog_ram , psram0)
...
SECTIONS
{
  ...
  .startaddrtable (0x80020040) : FLAGS(arl)
  {
    ApplStartAddrTable = .;
    KEEP (*(.startaddrtable))
    . = ALIGN(4);
  ...
  .interface_const (0x80020050) :
  {
   ...
   /*This section contains the data for BMH[1] CRC */
  .BMH_validate (0x80030000) :
  {
    __IF_CONST = .;
    KEEP (*(.BMH_validate));
    . = ALIGN(4);
  ...
  .text.MPU_ASIL_CODE : FLAGS(ax)
  {
    *(.text.MPU_ASIL_CODE)
    *(.text.STL_ASIL_CODE)
    *(.text.STL_ASIL_CODE_NO_OPTIMIZE)
    *(.text.CpuMpuExAccess)
    *(.text.IRTst_IRTab.*)
    *(.text.Mtl_TrapTab.*)
    *(.Mpu_traptab)
  ...
  .rodata.MPU_ASIL_CONST : FLAGS(a)
  {
    *(.rodata.MPU_ASIL_CONST)
    *(.rodata.STL_CONST_8BIT)
    *(.rodata.STL_CONST_16BIT)
    *(.rodata.STL_CONST_32BIT)
    *(.rodata.STL_CONST_FAR_UNSPECIFIED)
    *(.rodata.MTL_SRI_EDC_BUSERROR_TST)
    *(.rodata.CONFIGURATION_LT)
    *(.rodata.CONFIGURATION_PB)
    . = ALIGN(4) ;
  } > pfls0

  .rodata.MPU_QM_CONST : FLAGS(a)
  {
    *(.rodata.MPU_QM_CONST)
    . = ALIGN(4) ;
  } > pfls0

  .text.MPU_DEFAULT_CODE : FLAGS(ax)
  {
    __DSPR_DEFAULT_FLASH_START = .;
    *(.text)
    *(.text.*)
    . = ALIGN(8);
  } > pfls0

  .rodata.MPU_DEFAULT_CONST : FLAGS(a)
  {
    *(.rodata)
    *(.rodata.*)
    . = ALIGN(8) ;
    __DSPR_DEFAULT_FLASH_END = .;
  } > pfls0
  
   .code_ram (LCF_ISR15us_START):
  {
    __PSPR_ASIL_RAMCODE_START = .;
    . = ALIGN(8);
    *(.code_ram)
    *(.code_ram.*)
    . = ALIGN(8);
    __PSPR_ASIL_RAMCODE_END = .;
  } > prog_ram AT> pfls0


  .bss.MPU_QM_VAR_NOINIT (LCF_QM_VAR_START) :
  {
    __DSPR_QM_VAR_START = .;
    . = ALIGN(8) ;
    *(.bss.MPU_QM_VAR_NOINIT)
  } > dsram0 AT> pfls0

  .PMA_QM_VAR_INIT (LCF_QM_PMA_VAR_START):
  {
    __DSPR_QM_PMA_VAR_START = .;
    . = ALIGN(8) ;
    *(.data.PMA_QM_VAR_INIT)
    __DSPR_QM_PMA_VAR_END = .;
  } > dsram0 AT> pfls0

  .data.MPU_QM_VAR_INIT :
  {
    *(.data.MPU_QM_VAR_INIT)
    KEEP(*(.PMA_QM_VAR_INIT))
    . = ALIGN(8) ;
    __DSPR_QM_VAR_END = .;
  } > dsram0 AT> pfls0

  .bss.MPU_ASIL_VAR_NOINIT (LCF_ASIL_VAR_START) :
  {
    __DSPR_ASIL_VAR_START = .;
    . = ALIGN(8) ;
    *(.bss.MPU_ASIL_VAR_NOINIT)
    *(.bss.CpuMpu.*)
  } > dsram0 AT> pfls0


  .data.MPU_ASIL_VAR_INIT : 
  {
    *(.data.MPU_ASIL_VAR_INIT)
    *(.data.STL_RAM_UNSPECIFIED)
    *(.data.STL_RAM_UNSPECIFIED_NONZERO_INIT)
    *(.data.STL_RAM_32BIT_NONZERO_INIT)
    *(.data.STL_RAM_32BIT)
    *(.data.STL_RAM_NONZERO_INIT_8BIT)
    *(.data.STL_RAM_8BIT)
    . = ALIGN(8) ;
    __DSPR_ASIL_VAR_END = .;
  } > dsram0 AT> pfls0
     ...
     .rodata.GNUC : FLAGS(arl)
  {
    *(.rodata)
    ...
    PROVIDE(__clear_table = .) ;
      ...
      LONG(0 + ADDR(.bss.MPU_QM_VAR_NOINIT));    LONG(SIZEOF(.bss.MPU_QM_VAR_NOINIT));
      LONG(0 + ADDR(.bss.MPU_ASIL_VAR_NOINIT));    LONG(SIZEOF(.bss.MPU_ASIL_VAR_NOINIT));
      LONG(0 + ADDR(.MPU_DEFAULT_VAR));    LONG(SIZEOF(.MPU_DEFAULT_VAR));    LONG(SIZEOF(.MPU_DEFAULT_VAR));
      LONG(-1);                 LONG(-1);
      ...
      /*LONG(LOADADDR(.data.MPU_QM_VAR_INIT));    LONG(0 + ADDR(.data.MPU_QM_VAR_INIT));    LONG(SIZEOF(.data.MPU_QM_VAR_INIT));*/
      LONG(LOADADDR(.data.MPU_ASIL_VAR_INIT));    LONG(0 + ADDR(.data.MPU_ASIL_VAR_INIT));    LONG(SIZEOF(.data.MPU_ASIL_VAR_INIT));
      LONG(LOADADDR(.MPU_DEFAULT_VAR));    LONG(0 + ADDR(.MPU_DEFAULT_VAR));    LONG(SIZEOF(.MPU_DEFAULT_VAR));
      ...
      LONG(LOADADDR(.code_ram)); LONG(0 +ADDR(.code_ram)); LONG(SIZEOF(.code_ram));
  ...
  /*.testpattern (0x80018000) : FLAGS(arl)*/
  .testpattern (0x80058000) : FLAGS(arl)
  {
    KEEP(*(.MTL_PMU_ECC_EDC_TST_PF0))
  } > pfls0_tstPattern
  /*.testpattern_nc (0xa0018000) : FLAGS(arl)*/
  .testpattern_nc (0xa0058000) : FLAGS(arl)
  {
...
SECTIONS
{
  ...
  CORE_SEC(.PMUECCEDCTST_TVT_RAMCODE) (LCF_PMUECC_TVT_START):
  {
    . = ALIGN(256);
  ...
  /*[PIPB-10296] Wrong mapping to PMU ECC SafeTlib test memory sections*/

  CORE_SEC(.PFLASH_MON_ECCEDC_RAMCODE) (LCF_PMUECC_RAMCODE_START):
  {
...
*/
  ...
  .MPU_DEFAULT_VAR (LCF_DEFAULT_VAR_START):
  {
    __DSPR_DEFAULT_VAR_START = .;
    . = ALIGN(8) ;
    *(.sbss)
    *(.sbss.*)
    *(.bss)
