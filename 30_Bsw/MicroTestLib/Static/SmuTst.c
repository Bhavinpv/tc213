...
**                      recovery timer status is not set or                   **
**                      recovery time running during RT reg Initialization    **
...
)
{
  ...
  *TstSignature = (uint32)CRC32((uint32)TEST_ID_SMU_TST_RT,(uint32)TstSeed);
  ...
  else
  {
    ...
    {
    TempResult = (Sl_TstRsltType) SMUTST_RT_TIMEOUT_RTRUNERR;
    }
...
)
{
  ...
  Result= SmuTst_lRtInitRtRegs(SMUTST_RTC_RT01EN_RTDLOW);
  if(Result == SMUTST_RT_SUCCESS)
  {
  /* Initialize with error condition: SMU set alarm status error */
...
)
{
  ...
  Result = SmuTst_lRtInitRtRegs(SMUTST_RTC_RT01EN_RTDMAX);
  if(Result == SMUTST_RT_SUCCESS)
  {
  Result = (Sl_TstRsltType)SMUTST_RT_SMU_SETALMSTSERR;
...
** Traceability     : [cover parentID=]          [/cover]                     **
**                                                                            **
** Syntax           : static Sl_TstRsltType SmuTst_lRtInitRtRegs                             **
...
** Parameters (out) : None                                                    **
**                                                                            **
** Return value     : SMUTST_RT_TIMEOUT_RTRUNERR:Initialization failed because**
**                        recovery timer is running                           **         
**                  : SMUTST_RT_SUCCESS : Initialization success              **                  
**                                                                            **
...
static Sl_TstRsltType SmuTst_lRtInitRtRegs(const uint32 SmuRtcValue)
{
  Sl_TstRsltType Result = (Sl_TstRsltType)SMUTST_RT_TIMEOUT_RTRUNERR;
  uint32 TempRecTimeStatus;
  /* Read recovery timer 0 and 1 status bit field */
  TempRecTimeStatus = (uint32)(SMU_STS.U & SMU_STS_RTS_MASK);
