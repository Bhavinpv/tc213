...
/*For clearing the Pflash double bit error flag*/
#define FLASH0_FSR_PFDBER                   (15U)
...
)
{
  ...
  #ifdef PFLASH_FIFO_BUFFERFULL_TEST_ENABLE
  /* Clear Doublebit PFLASH ECC error flag: */
  FLASH0_FSR.U = (uint32)(((uint32)1U)<<(FLASH0_FSR_PFDBER));
  
  ISYNC();
  
  /* Read additional ten double-bit error page for Fifo buffer full test
  of given flash bank: */
  for (i = 0U; i < PFLASH_MON_CBAB_FIFO_DEPTH; i++)
  {
    Dummy = TestPattern->DBEPage[i].Data[0U];
    /* Doublebit PFLASH ECC error flag should now be set, if patterns
    * have already been programmed:
    */
    if (1U == FLASH0_FSR.B.PFDBER)
    {
    /* Clear error flag: */
    FLASH0_FSR.U = (uint32)(((uint32)1U)<<(FLASH0_FSR_PFDBER));
  ...
  /* Verify 4 patternsets and additional 10 double bit error pages 
  * are already programmed*/
  if (14U == ErrorPatternCount)
  #else
  /* Verify 4 patternsets are already programmed*/
