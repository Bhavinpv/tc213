...
/*Mask for SMU FSP mode*/
#define SMU_FSP_MODE_MASK (0xFFFFFF9FU)

/*Time Switching protocol for SMU FSP mode*/
#define SMU_TIMESWITCHING_MODE (0x00000040U)

#define SMU_STS_RTS_MASK (0x50000U) 
...
)
{
  ...
  if(RetVal == (Std_ReturnType)E_OK)
  {
    ...
    TempRecTimeStatus = (uint32)(SMU_STS.U & SMU_STS_RTS_MASK);
    ...
    if(TempRecTimeStatus == 0U)
    {
      ...
      /*Check if Time Switching Protocol is already enabled*/
      if((SMU_FSP.U & (~SMU_FSP_MODE_MASK)) == SMU_TIMESWITCHING_MODE)
      {
        /* Disable Time Switching Protocol by setting FSP in Bi-stable Protocol*/
        SMU_FSP.U = SMU_FSP.U & SMU_FSP_MODE_MASK;
        /*Hardware Errata SMU_TC.012 Fix Read back register FSP twice 
         to wait until Bi-Stable protocol mode is active*/
		dummy = SMU_FSP.U ;
		dummy = SMU_FSP.U ;
		
      }
      /* Write PRE1, PRE2, PEs, TFSP_HIGH values*/
      SMU_FSP.U = (unsigned_int) ((ConfigPtr->FSPCfg) & SMU_FSP_MODE_MASK);
      /*Check if need to enable Time Switching Protocol */
      if(((ConfigPtr->FSPCfg) & (~SMU_FSP_MODE_MASK)) \
                              ==SMU_TIMESWITCHING_MODE)
      {
        /* Enable Time Switching Protocol*/
        SMU_FSP.U = SMU_FSP.U | SMU_TIMESWITCHING_MODE;
      }
